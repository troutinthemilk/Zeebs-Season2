...
```{r, include = TRUE}
double.dat <- read_xlsx(path="../Data/Season2/Encounters - Quadrats (Responses).xlsx", sheet=1) %>% dplyr::select(-1) #note this is the quadrat data...
...
```{r}
double.dat      <- read_xlsx(path="../Data/Season2/Encounters - Double observer - no distance (Responses).xlsx", sheet=1) %>% dplyr::select(-1)
double.dat$`Number of mussels in cluster`[is.na(double.dat$`Number of mussels in cluster`)] <- 0
##if there are no mussels in the cluster
```
```{r}
transect.dat      <- read_xlsx(path="../Data/Season2/Transect datasheets (Responses).xlsx", sheet=1) %>% filter(`Survey type` == "Double observer no distance") %>% dplyr::select(`Transect number`, `Transect length (if transect survey)`, `Lake name:`)
dim(transect.dat)

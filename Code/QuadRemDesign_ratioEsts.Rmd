...
# lbout
# lbout <- gmultmix(~area, ~1, ~ area, data = lbFrame, mixture = "P", starts = c(5,0,1, .5))  # log area produces NAs
# lbout
```
...
```{r}
svy.out$strata
ests.df$Lake[(1:3)+5*2] <- svy.out$strata
ests.df$method[(1:3)+5*2] <- "Double no distance, multiNegBinom"
ests.df$Dhat[(1:3)+5*2] <- c(abund.ball/sum(covDf.b$area), abund.fall/sum(covDf.f$area),NA) # abund.lball/sum(covDf.lb$area))
ests.df$SE[(1:3)+5*2] <- c(abund.ball.se/sum(covDf.b$area), abund.fall.se/sum(covDf.f$area), abund.lball.se/sum(covDf.lb$area))
ests.df$detProb[(1:3)+5*2] <- c(
                     mean(backTransform(linearComb(bout, type = "det", coefficients = matrix(c(rep(1,3), covDf.b$area), byrow=FALSE, nrow=nrow(covDf.b))))@estimate),

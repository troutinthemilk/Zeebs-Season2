...
library(mrds)
library(knitr)
ests.df <- data_frame(Lake = rep(NA, 3*4), Dhat = rep(NA, 3*4), SE = rep(NA, 3*4), method = rep(NA,3*4)) 
```
...
bout <- multinomPois(~1 ~ area , data = bFrame)
bout
bout@estimates
fout <- multinomPois(~1 ~1 , data = fFrame)
fout
lbout <- multinomPois(~1 ~ area , data = lbFrame)
```
...
#abund.b<- backTransform(linearComb(bout, type = "state", coefficients = matrix(covDf.b$area, nrow=nrow(covDf.b) )))  # no intercept
abund.b
abund.b1m<- backTransform(linearComb(bout, type = "state", coefficients = c(1, 1) ))  # intercept
#abund.b1m<- backTransform(linearComb(bout, type = "state", coefficients = c(1) ))  # no intercept
abund.b1m
...
abund.lb<- backTransform(linearComb(lbout, type = "state", coefficients = matrix(c(rep(1,3), covDf.lb$area), byrow=FALSE, nrow=3) )) # intercept
#abund.lb<- backTransform(linearComb(lbout, type = "state", coefficients = matrix(covDf.lb$area, nrow=nrow(covDf.lb) )))  # no intercept
abund.lb
abund.lb1m<- backTransform(linearComb(lbout, type = "state", coefficients = c(1, 1) ))  # intercept
#abund.lb1m<- backTransform(linearComb(lbout, type = "state", coefficients = c(1) ))  # no intercept
abund.lb1m
# add transect estimates
...
ests.df$detProb <- c(1,1,1,1,1,1,
                     backTransform(linearComb(bout, type = "det", coefficients = 1))@estimate,
                     backTransform(linearComb(fout, type = "det", coefficients = 1))@estimate, 
                     backTransform(linearComb(lbout, type = "det", coefficients = 1))@estimate, 
                     NA,NA,NA)
```

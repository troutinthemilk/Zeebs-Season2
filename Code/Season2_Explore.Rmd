...
index             <- 1
LB.distance.est   <- distance.dens.est(curr.lake[index], curr.lake2[index], var.type=var.type)
LB.double.est     <- double.dens.est(curr.lake[index], curr.lake2[index], var.type=var.type)
LB.quadrat.est    <- quadrat.dens.est(curr.lake[index], curr.lake2[index], var.type=var.type)

LB.distance.est.jack   <- distance.dens.est(curr.lake[index], curr.lake2[index], var.type="jack")
...
LB.distance.est.design   <- distance.dens.est(curr.lake[index], curr.lake2[index], var.type="design")
...
LB.distance.est.model    <- distance.dens.est(curr.lake[index], curr.lake2[index], var.type="model")
...
index            <- 2
LBL.distance.est  <- distance.dens.est(curr.lake[index], curr.lake2[index], var.type)
LBL.double.est    <- double.dens.est(curr.lake[index], curr.lake2[index], var.type)
...
index            <- 3
LF.distance.est  <- distance.dens.est(curr.lake[index], curr.lake2[index], var.type)
...
There are a number of ways to estimate the variance in the surveyed data. We tested three on Lake Burgan below. The total variance in the double observer methods is given by $\mathrm{Var}(\hat{D})= \hat{D}^2\left(\frac{\mathrm{Var}(n)}{n^2} + \frac{\mathrm{Var(\mathrm{E}(s)}}{\mathrm{E}(s)^2} + \frac{\mathrm{Var}(a \hat{P})}{ (a \hat{P})^2 } \right)$. For a design-based estimate with $K$ transects each of length $l_k$ the first variance component is $\mathrm{Var}(n) = \frac{K}{K-1} \displaystyle\sum_{k=0}^K l_k^2 \left(\frac{n_k}{t_k l_k} - \frac{\sum n_k}{\sum l_k} \right)^2$.
We also tested a jack-knife estimate of the mean and variance. Denoting the mean density with the $i^{\mathrm{th}}$ transect held out as $\bar{D}_{-i}$, the jacknife mean and variance are $\bar{D} = \frac{1}{K}\sum_i^K \bar{D}_{-i}$ and $\mathrm{Var}({\bar{D}}) = \frac{K-1}{K}\sum_i^K \left( \bar{D}_{-i} - \bar{D}\right)^2$. <!-- We also implemented the jack-knife bias reduction on the mean (*to be added*). -->Finally, we applied a model-based estimation process where transect counts were modeled as a negative binomial random variable. The offset in the model was the transect survey area times the estimated probability of detection in the transect.
...
library(RColorBrewer)
col.vec <- brewer.pal(3, "Set1")
dhat.vec    <- c(LB.distance.est.design$Dhat, LB.distance.est.jack$Dhat, LB.distance.est.model$Dhat)
dhat.se.vec <- c(LB.distance.est.design$Dhat.se, LB.distance.est.jack$Dhat.se, LB.distance.est.model$Dhat.se)

plotCI(x=rep(1,3)+c(-0.1,0,0.1), y=dhat.vec, ui=dhat.vec+2*dhat.se.vec, li=dhat.vec - 2*dhat.se.vec, xaxt = 'n', ylab="Estimated density", xlab="Estimator", col=col.vec, pch=19, lwd=2, cex=1.3, sfrac=0, cex.lab=1.3, main="Lake Burgan", ylim=c(0,1), xlim=c(0.75,3.25))
axis(1, at=c(1,2,3), labels=c("Distance", "Double observer", "Quadrat"))

dhat.vec    <- c(LB.double.est.design$Dhat, LB.double.est.jack$Dhat, LB.double.est.model$Dhat)
dhat.se.vec <- c(LB.double.est.design$Dhat.se, LB.double.est.jack$Dhat.se, LB.double.est.model$Dhat.se)

plotCI(x=rep(2,3)+c(-0.1,0,0.1), y=dhat.vec, ui=dhat.vec+2*dhat.se.vec, li=dhat.vec - 2*dhat.se.vec, xaxt = 'n', ylab="Estimated density", xlab="Design", col=col.vec, pch=19, lwd=2, cex=1.3, sfrac=0, cex.lab=1.3, main="Double observer survey", ylim=c(0,1), add=T)

dhat.vec    <- c(LB.quadrat.est.design$Dhat, LB.quadrat.est.jack$Dhat, LB.quadrat.est.model$Dhat)
dhat.se.vec <- c(LB.quadrat.est.design$Dhat.se, LB.quadrat.est.jack$Dhat.se, LB.quadrat.est.model$Dhat.se)

plotCI(x=rep(3,3)+c(-0.1,0,0.1), y=dhat.vec, ui=dhat.vec+2*dhat.se.vec, li=dhat.vec - 2*dhat.se.vec, xaxt = 'n', ylab="Estimated density", xlab="Estimator", col=col.vec, pch=19, lwd=2, cex=1.3, sfrac=0, cex.lab=1.3, main="Quadrat survey", ylim=c(0,1), add=T)
...
axis(1, at=c(1,2,3), labels=c("Burgan", "Little Birch", "Florida"))
text(x=c(1,1, 2,2, 3,3) + 2*c(-0.05, 0.05), phat.vec-0.11, labels=ss.vec, col=col.vec[-3])

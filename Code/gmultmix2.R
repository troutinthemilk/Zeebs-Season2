source("https://raw.githubusercontent.com/cran/unmarked/master/R/unmarkedEstimate.R")
source("https://raw.githubusercontent.com/cran/unmarked/master/R/getDesign.R")
 ...
 #   stop("Data is not of class unmarkedFrameGMM.")
  ...
  for(i in 1:M) {
    ...
    for(t in 1:T) {
      ...
      if(!all(naflag[i,t,])) {
        kmyt[i,t,] <- k - yt[i,t]  # really big when y big
        lfac.kmyt[i, t, fin[i,]] <- lgamma(kmyt[i, t, fin[i,]] + 1)  # really big
      }
    }
  }
  ...
  nll <- function(pars) {
    ...
    switch(mixture,
           P = f <- sapply(k, function(x) dpois(x, lambda, log = TRUE)),
           NB = f <- sapply(k, function(x) dnbinom(x, mu=lambda,
                                                   size=exp(pars[nP]), log = TRUE)))
    ...
    for(i in 1:M) {
      ...
      g[i,] <- rowSums(A)   #exp(rowSums(A))
    }
    #f[!fin] <- g[!fin] <- 0
    expsumfg <- exp(f + g)
    expsumfg[!fin] <- 0
    ll <- rowSums(expsumfg)    # rowSums(f*g)
    -sum(log(ll))
  }
  
  # loglik = log(sum(f*g))
  # log lik = log(sum(exp(log(f) + log(g))))
  #nll(c(3.1399659,   .1026389, -5.1867414,   .650632,5))
  #nll(rep(0,5))
  
  #pars <- c(3.1399659,   .1026389, -5.1867414,   .650632,5)

...
The remainder of the tutorial assumes that you have been able to install this package. All the data and functions you will need to complete the tutorial are in this package. To load the library type:
...
where $\hat{P}$ denotes the estimated probability of detecting a zebra mussel by one or both observers.
...
JF: **Jake - check the comments/documentation, below. Also, do we need this line: #remove.ests[1]sqrt(sum(data.frame(remove.out)[2,]^2, na.rm = TRUE))**
...
**JAKE: this SE seems really low - is it accurate?**
...
The average probability of detecting a mussel by one of the observers is $\hat{P}=0.58$ with a standard error of $\sigma_\hat{P}=0.06$.  Below, we overlay the estimated detection model on the histogram of detection distances made by the two observers. 
```{r, warning=F}

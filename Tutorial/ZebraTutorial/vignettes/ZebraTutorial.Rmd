...
The variance in the total counts can be estimated using the `dht` function in the `mrds` package. We need to create two tables to link the survey effort and surveyed area to the actual count data. This is done below, then we run the `dht` function to get the density and it's standard error.
...
Reading off the output table We get an estimate for $\hat{D}=0.25$ with a standard error of $0.06$.

...
```{r}
area    <- DistanceTransect$`Transect length` #length of transects
...
Phat    <- 0.5312114 #from distance model
VarP    <- 0.0659363^2 #from distance model

#get density esimate and its standard error.
Dhat    <- sum(counts)/(w*sum(area)*Phat)
VarX    <- sum(area)*sum(area*(counts/area - sum(counts)/sum(area))^2)/(n-1)

seNL <- sqrt( n/(n-1)/sum(area)^2*sum(area^2*(counts/area - sum(counts)/sum(area))^2) )

cvNL <- seNL/(sum(counts)/sum(area))
VarD <- Dhat^2*(cvNL^2 + VarP/Phat^2)
```

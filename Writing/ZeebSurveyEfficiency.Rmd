...
a <- 30
#epsilon <- 145.41/60^2 #marshmallow data give epsilon=145 (seconds)
...
enc.predict   <- predict(tenc.length, newdata=newDat, se=TRUE)
...
plotCI(Density.est, enc.predict$fit/60^2, uiw=2*enc.predict$se.fit/60^2, sfrac=0, col=col.vec, pch=19, xlab="Density", ylab="Search time", log="xy", ylim=c(0.1, 3.3))
...
* Can approximate double observer search with a single observer, though not exactly clear on how to set $m$, the sample size of the single observers calibration to estimate $\var{\hat{P}$.
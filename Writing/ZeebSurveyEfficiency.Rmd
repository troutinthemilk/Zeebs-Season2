...
mean.quad.sample <- function(ts, D, a, tH, tT, alpha, t0, offset=0) {
...
mean.doub.sample <- function(ts, D, a, beta, tH, tT, alpha, m, t0) {
...
th.dub  <-  (coef(tenc.length)[3] + coef(tenc.length)[6])/60^2 
th.quad <-  (coef(tenc.length)[3] + coef(tenc.length)[7])/60^2 

ts.dist <- sum(coef(tenc.length)*c(1, 30, 0, 0, 0, 0, 0))/60^2
ts.dub  <- sum(coef(tenc.length)*c(1, 30, 0, 1, 0, 0, 0))/60^2
ts.quad  <- sum(coef(tenc.length)*c(1, 30, 0, 0, 1, 0, 0))/60^2

t0.dist <- sum(coef(tset.length)*c(1, 30, 0, 0))/60^2 #+  15/60
t0.dub  <- sum(coef(tset.length)*c(1, 30, 1, 0))/60^2 #+  15/60
t0.quad <- sum(coef(tset.length)*c(1, 30, 0, 1))/60^2 #+  15/60
...
for(i in 1:length(D.vec)) {
  ...
  cv.dist.opt.ts[i] <- optimize(cv.doub.sample, interval=c(0, tT), tol=.Machine$double.eps, tT=mean(tT.dist, tT.dub, tT.quad), a=60, D=D.vec[i], beta=dist.beta, alpha=alpha.fit[1], tH=th.dub, t0=t0.dub)$minimum
  cv.dist.opt[i]    <- cv.doub.sample(ts=cv.dist.opt.ts[i], a=60, D=D.vec[i], beta=dist.beta, tH=th.dub, tT=tT, alpha=alpha.fit[1], m=NULL, t0=t0.dist)
...
for(i in 1:length(D.vec)) {
  ...
  cv.dist.opt.ts[i] <- optimize(cv.doub.sample, interval=c(0, tT), tol=.Machine$double.eps, tT=mean(tT.dist, tT.dub, tT.quad), a=60, D=D.vec[i], beta=dist.beta, alpha=alpha.fit[2], tH=th.dub, t0=t0.dub)$minimum
  cv.dist.opt[i]    <- cv.doub.sample(ts=cv.dist.opt.ts[i], a=60, D=D.vec[i], beta=dist.beta, tH=th.dub, tT=tT, alpha=alpha.fit[2], m=NULL, t0=t0.dist)
...
for(i in 1:length(D.vec)) {
  for(j in 1:length(alpha.vec)) {
    dub.mat[i,j]    <- cv.doub.sample(ts=ts.dist, a=30, D=D.vec[i], beta=double.beta, tH=th.dub, tT=mean(tT.dist, tT.dub, tT.quad), alpha=alpha.vec[j], m=NULL, t0=t0.dub)
...
```{r optSearchtimeMean, echo=F, fig.asp=1, out.width = '50%', fig.cap="\\label{fig:optStrat}Illustration of the optimal search time parameters from Lake  Burgen and a total time budget of 8 hours."}
library(akima)
tT <- 16
num <- 100
...
for(i in 1:length(D.vec)) {
  for(j in 1:length(alpha.vec)) {
    dist.opt.ts[i,j] <- optimize(mean.doub.sample, interval=c(0, tT), tol=1e-10, tT=tT, a=30, D=D.vec[i], beta=dist.beta, alpha=alpha.vec[j], tH=th.dist, t0=t0.dist, maximum=TRUE)$maximum
  }
}
...
for(i in 1:length(D.vec)) {
  for(j in 1:length(alpha.vec)) {
    dist.opt.ts[i,j] <- optimize(cv.doub.sample, interval=c(0, tT), tol=1e-10, tT=tT, a=30, D=D.vec[i], beta=dist.beta, alpha=alpha.vec[j], tH=th.dist, t0=t0.dist)$minimum
  }
}
